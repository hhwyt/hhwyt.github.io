...
- DeleteOnly
  - 如果是 Index 的 Schema，只对 delete 操作可见。更进一步，由于 update 操作会拆分为 delete 操作和 insert 操作，只对这个 delete 操作可见，忽略掉 insert 操作。
...
根据上图可知，Index 的 Schema 如果是 Absent 状态，Index 不可读写；DeleteOnly 状态，Index 仅支持删除；WriteOnly 状态，Index 仅支持写；Public 状态，可读可写。「三步走」方案，一步一个脚印，循序渐进地完成了一个 Index 从无到有的创建过程。（reorg 即 reorganization，是指数据重新组织，后文会介绍）。
...
但是，DeleteOnly 状态和 Public 状态是不兼容的。与上面两个状态类似，这两个状态间的转换也不会存在孤儿数据的问题。但是，由于 Public 状态下的 Schema 对 read 操作可见，所以要求数据必须是完整的。然而 DeleteOnly 状态直接转换到 Public 状态会导致数据不完整，原因和上文分析过的从 Absent 状态转换到 Public 状态一样，不再赘述。
